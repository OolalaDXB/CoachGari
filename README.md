...
node --experimental-strip-types scripts/test-audience-sync.mjs         # offline: counting window, /admin exclusion, country-only location, the Instagram token boundary, 79 checks
```
...
node --experimental-strip-types scripts/test-audience-sync.mjs  # AUDIENCE_SYNC_TESTS ok=79
```
