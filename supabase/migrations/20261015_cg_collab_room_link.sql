-- =====================================================================
-- Coach Gari — Collaborations: put the private room link in every notification
--
-- Notification emails could not carry a clickable link because only the SHA-256
-- of the room token was stored. This keeps the hash for auth AND stores the raw
-- token (room_token) so the producers can build the room URL
-- (https://coachgari28.com/c/<token>) into the customer-facing emails, and the
-- admin can copy / re-share the link without regenerating it. The room holds
-- commercial deal data behind a 256-bit unguessable URL (no health/credentials),
-- so storing the token to enable magic-link emails is the accepted trade-off.
-- Additive, forward-only; the negotiation flow itself is unchanged.
-- =====================================================================

alter table public.collaboration_deals add column if not exists room_token text;

-- Intake now records room_token and puts the room link in the acknowledgement email.
...
begin
  ...
  insert into public.collaboration_deals (public_ref, crm_contact_id, company, contact_name, contact_email, contact_phone, contact_url,
      ...
      intake_budget_amount, intake_budget_currency, intake_offer, status, access_token_hash, room_token, source, created_by)
  values (public.collab_new_ref(), cid, nullif(btrim(coalesce(p ->> 'company','')),''), v_name, v_email, v_phone,
      ...
      'new', encode(extensions.digest(tok, 'sha256'), 'hex'), tok, 'public', 'public')
  returning * into d;
...
-- Regenerate stores the new raw token too.
...
begin
  ...
  tok := encode(extensions.gen_random_bytes(32), 'hex');
  update public.collaboration_deals set access_token_hash = encode(extensions.digest(tok,'sha256'),'hex'), room_token = tok, token_revoked_at = null where id = p_id returning * into d;
...
-- Coach proposal email carries the room link.
  ...
  if d.contact_email is not null then
    perform public.email_queue('collab_proposal', d.contact_email,
      jsonb_build_object('public_ref', d.public_ref, 'name', d.contact_name, 'version', nextv, 'monetary_amount', v_amt, 'currency', v_ccy,
                         'room_url', 'https://coachgari28.com/c/' || d.room_token),
...
-- Acceptance from the room: the brand's confirmation carries the room link.
...
begin
  ...
  if d.contact_email is not null then
    perform public.email_queue('collab_accepted', d.contact_email,
      jsonb_build_object('public_ref', d.public_ref, 'name', d.contact_name, 'version', pr.version_number, 'by', 'you', 'room_url', 'https://coachgari28.com/c/' || d.room_token),
...
-- Coach accepts the brand's counter: the brand's confirmation carries the room link.
...
begin
  ...
  if d.contact_email is not null then
    perform public.email_queue('collab_accepted', d.contact_email,
      jsonb_build_object('public_ref', d.public_ref, 'name', d.contact_name, 'version', pr.version_number, 'by', 'Coach Gari', 'room_url', 'https://coachgari28.com/c/' || d.room_token),
...
-- Payment-ready email carries the room link (where the brand pays).
...
-- Admin serializer exposes the room URL (admin only) so the deal view can copy / share it.
...
begin
  ...
  return jsonb_strip_nulls(jsonb_build_object(
    ...
    'room_url', case when p_admin and d.room_token is not null and d.token_revoked_at is null then 'https://coachgari28.com/c/' || d.room_token else null end,
