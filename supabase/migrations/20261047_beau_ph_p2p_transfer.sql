-- =====================================================================
-- BEAU PH — peer-to-peer transfer as a first-class capability
--
-- BEAU PH is meant to be extracted and sold as a connectable module, so it has
-- to be able to express every shape of payment its future merchants actually
-- use — not only the ones Coach Gari needs today. A peer-to-peer transfer
-- (PayPal "friends and family", a Wise personal send) is a real shape: splitting
-- a cost, reimbursing an expense, a gift between individuals, a club collecting
-- from its own members.
--
-- WHAT MAKES IT SAFE TO SHIP. The thing that gets a receiving account limited is
-- not the rail, it is using it for a COMMERCIAL payment. So the rail exists in
-- full, and the hub refuses to offer it for a commercial request:
--
--   * a new intent, 'personal', for a transfer that is not a sale;
--   * a new capability, 'p2p_transfer';
--   * capabilities can now declare which intents they serve (intents text[],
--     null = any), and the matrix refuses a capability whose intents do not
--     include the request's. That is general: any capability can be restricted
--     this way, and p2p_transfer is simply the first that needs it.
--
-- A merchant still has to opt in twice — the capability in merchant_methods.
-- capabilities and 'personal' in merchant_methods.intents — so it can never
-- appear by default on a host that sells something.
--
-- OPERATOR-CONFIRMED, ALWAYS. A friends-and-family send produces no Orders v2
-- order and no webhook we can verify: there is nothing to confirm it with. The
-- capability is therefore confirmation = 'operator' on every provider, and the
-- request records capability = 'p2p_transfer' and intent = 'personal', so the
-- ledger says exactly what kind of payment happened and on whose word.
-- =====================================================================
...
/* 'personal' is a transfer that is not a sale. It is deliberately not a synonym
   for 'support': a tip to a business is still commercial. */
create or replace function beau_ph.is_intent(p text)
returns boolean language sql immutable set search_path = '' as $$
  select p in ('service','package','support','other','personal')
$$;

-- ---------- 2. a capability can serve only some intents ----------
alter table beau_ph.provider_capabilities add column if not exists intents text[];
comment on column beau_ph.provider_capabilities.intents is
  'Which request intents this capability may serve. Null = any. Used to keep a peer-to-peer rail off a commercial request.';

-- ---------- 3. the matrix honours it ----------
create or replace function beau_ph.method_matrix(p_merchant_key text, p_country text default null, p_currency text default null,
                                                 p_runtime jsonb default '{}'::jsonb, p_platform text default null,
                                                 p_initiated_by text default 'customer', p_intent text default null)
...
-- ---------- 4. the rails that have one ----------
insert into beau_ph.provider_capabilities (provider_key, capability, readiness, confirmation, platforms, initiated_by, handoff, intents, notes) values
  ('paypal', 'p2p_transfer', 'available', 'operator', null, 'any', false, '{personal}',
   'PayPal personal transfer (friends and family). Non-commercial requests only: the intent must be personal, and the merchant must opt in. No Orders v2 order and no webhook exists for this, so an authorised operator confirms receipt. Sending a commercial payment this way breaches PayPal''s terms and removes protection for both sides.'),
  ('wise', 'p2p_transfer', 'available', 'operator', null, 'any', false, '{personal}',
   'Wise personal transfer between individuals. Non-commercial requests only. An authorised operator confirms receipt; Wise reports nothing back.')
on conflict (provider_key, capability) do update
  set readiness = excluded.readiness, confirmation = excluded.confirmation,
      intents = excluded.intents, notes = excluded.notes;

/* The commercial capabilities of both rails are explicitly marked as serving
   every intent EXCEPT 'personal': a sale is not a personal transfer either, and
   saying so keeps the two directions symmetrical instead of relying on one. */
update beau_ph.provider_capabilities
   set intents = '{service,package,support,other}'
 where provider_key in ('paypal','wise') and capability <> 'p2p_transfer';
