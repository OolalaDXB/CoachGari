...
|---|---|---|
| `supabase/tests/beau_ph_contract.sql` | **BEAU PH contract** — generic core with a throw-away merchant, the Coach Gari host adapter, in-person / SoftPOS, the multi-rail race, multi-tenant isolation and the test/live mode gate | `BEAU_PH_TESTS ok=84 fail=0` |
...
- **payment mode (§20)** — a live merchant refuses a test runtime (`mode_mismatch`), accepts a live one, and refuses a runtime that is live-but-not-configured (`key_mode_mismatch`); a test-mode event on a live request is refused and kept as evidence, a live one normalizes; the Coach Gari merchant (intended live) offers no card on its report page to a test runtime and does to a live one; a test webhook on a live host order is ignored (`rejected:mode_mismatch`) and the live one pays it once

Suites pin the Coach Gari merchant to **test** mode inside their rolled-back transaction, so they stay green whatever the production mode is.
