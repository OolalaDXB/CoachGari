...
- Payment mode gate (replaces the categorical live refusal of CHECK-LICENCE-001): `PAYMENTS_MODE` (`test` | `live`) is the deployment's declared mode. The Stripe adapter reports `configured` only when the key's mode equals it (`sk_test_` under test, `sk_live_` under live); a mismatch is `key_mode_mismatch`, an unset/unknown mode is `payments_mode_unset` — payment creation is refused rather than guessed. `runtime()` never exposes a key or a prefix, only `{configured, mode, reason}`. The webhook refuses an event whose `livemode` differs from `PAYMENTS_MODE` (`mode_mismatch`, both directions) and refuses everything when the mode is unset. The DB core independently refuses evidence whose `livemode` does not match the merchant's own mode, in both directions, and eligibility refuses a runtime whose mode differs from the merchant's — so the deployment mode and the merchant's intended mode must agree before a single Checkout Session can exist.
