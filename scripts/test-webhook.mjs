/* CG-003 — live probe of the deployed stripe-webhook function (run from a laptop).
   ...
   (never in git):
     STRIPE_WEBHOOK_SECRET=whsec_... [PAYMENTS_MODE=live] node scripts/test-webhook.mjs
   PAYMENTS_MODE (default test) must match the deployment's setting: probe
   events carry that livemode, and one probe checks that the OTHER mode is
   refused. Every event sent is of an unhandled type or unknown session, so
   nothing is confirmed or paid; accepted events are recorded as `ignored`.
   Full end-to-end (real Stripe → real signature) is `node scripts/test-checkout.mjs --wait`
   or `stripe trigger checkout.session.completed` with the Stripe CLI (test mode). */
import { signForTest } from '../beau-ph/providers/stripe/signature.js';
import { randomUUID } from 'node:crypto';

const URL = process.env.WEBHOOK_URL || 'https://acrjrlgeeyseyolmofuq.supabase.co/functions/v1/stripe-webhook';
const SECRET = process.env.STRIPE_WEBHOOK_SECRET;
const LIVE = (process.env.PAYMENTS_MODE || 'test').toLowerCase() === 'live';
if (!SECRET) { console.error('Set STRIPE_WEBHOOK_SECRET (the endpoint signing secret from the Stripe dashboard).'); process.exit(2); }
let ok = 0, fail = 0;
const ev = (over = {}) => JSON.stringify({ id: 'evt_probe_' + randomUUID().slice(0, 8), object: 'event', type: 'customer.created', livemode: LIVE, data: { object: { id: 'cus_probe' } }, ...over });
...
console.log(`INFO  probing as PAYMENTS_MODE=${LIVE ? 'live' : 'test'}`);
let b = ev();
...
await send('tampered payload (livemode flipped after signing) → 400', b.replace(`"livemode":${LIVE}`, `"livemode":${!LIVE}`), good, 400, 'bad_signature');
...
const other = ev({ livemode: !LIVE });
await send(`correctly signed ${LIVE ? 'TEST' : 'LIVE'}-mode event → 400 mode_mismatch`, other, await signForTest(other, SECRET, now), 400, 'mode_mismatch');
